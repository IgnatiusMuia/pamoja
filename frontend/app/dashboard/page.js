...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="bg-gradient-to-br from-emerald-700 to-sky-800 text-white rounded-3xl p-8 mb-8">
        ...
        <p className="text-emerald-100 mt-1">
          {user.role === "companion"
            ...
            : "Traveller dashboard — your bookings, messages and reviews."}
        </p>
        {user.role === "companion" && !user.is_approved && (
          <div className="mt-4 bg-orange-500/20 border border-orange-300/40 rounded-xl px-4 py-3 text-sm">
            ⏳ Your companion profile is <strong>pending approval</strong>. Our team reviews profiles to keep
            the community safe — you'll appear in search as soon as you're approved.
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        {user.role === "companion" ? (
          <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
            <p className="text-xs font-bold text-stone-400 uppercase">Earnings (payouts)</p>
            <p className="text-2xl font-extrabold text-orange-600 mt-2">{ksh(earnings)}</p>
          </div>
        ) : (
          <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
            <p className="text-xs font-bold text-stone-400 uppercase">Spent</p>
            <p className="text-3xl font-extrabold text-orange-600 mt-1">{ksh(completed.reduce((s, b) => s + b.total_kes, 0))}</p>
          </div>
        )}
        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
      ...
      {user.role === "companion" && companionProfile && (companionProfile.activity_types?.length === 0 || !companionProfile.tagline) && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5 mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="font-bold text-orange-800">Complete your companion profile to get booked 🚀</p>
          ...
          <Link href="/dashboard/companion-profile" className="bg-orange-500 text-white font-bold px-5 py-2.5 rounded-xl">
            Set up now
          </Link>
        </div>
      )}
      ...
      {bookings.length === 0 ? (
        <div className="text-center py-16 bg-white border border-stone-200 rounded-2xl">
          ...
          {user.role === "traveler" ? (
            <Link href="/search" className="inline-block bg-emerald-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-emerald-700">
              Find a companion
            </Link>
          ) : (
            <Link href="/dashboard/companion-profile" className="inline-block bg-emerald-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-emerald-700">
      ...
      <div className="mt-8 grid sm:grid-cols-2 gap-4">
        ...
        <Link href="/dashboard/profile" className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm hover:border-emerald-300 transition-colors">
          <p className="font-bold">🛡️ Profile & safety</p>
